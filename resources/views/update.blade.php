...
<div class="w-full h-[calc(100vh-72px)] flex items-center justify-center">
    <form action="/update/{{ $recipe->id }}" method="POST" class=" shadow-[rgba(255,165,0,_0.1)_0px_5px_20px] w-[600px]
    p-5
    rounded"
          enctype="multipart/form-data">
        @csrf
        <h1 class="my-5 text-3xl font-bold text-center text-orange-500 uppercase tracking-wider">update recipe</h1>
        <div>
            <label for="">Title recipe</label>
            <input required type="text" name="title" value="{{ $recipe->title }}" class="block bg-gray-100 w-full
            py-2.5
            outline-none
            px-5
            my-1.5
        ...
        <div class="my-4">
            ...
            <textarea rows="3" name="ingts"
                      ...
                      placeholder="Enter ingredient">{{ $recipe->ingredient }}</textarea>
        </div>
        <div class="my-4">
            <label for="">Description recipe</label>
            <textarea required rows="3" name="desc"
                      class="block resize-none bg-gray-100 w-full my-1.5 rounded-lg px-5 outline-none py-2.5 focus:ring-2 focus:ring-orange-300"
                      placeholder="Enter description">{{ $recipe->description }}</textarea>
        </div>
        <div>
            <label for="category"></label>
            <select multiple required name="category" id="category" class="py-2.5 px-3 w-full bg-gray-100 text-sm
            rounded-xl
            outline-none
        ...
        <div class="flex items-center mt-6 justify-center">
            <input type="submit" value="Update" class="block cursor-pointer bg-orange-500 rounded-3xl text-white py-2
            .5 w-[180px]">
        </div>
    </form>
</div>
